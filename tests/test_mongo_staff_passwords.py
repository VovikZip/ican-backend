...
async def test_superadmin_can_reset_any_staff_password_without_logging_it():
    ...
    response = await reset_staff_password(2, StaffPasswordReset(new_password="new-password"), db, actor)
...
async def test_superadmin_can_reset_own_password_and_must_sign_in_again():
    ...
    response = await reset_staff_password(1, StaffPasswordReset(new_password="new-password"), db, actor)
...
async def test_non_superadmin_cannot_reset_passwords():
    with pytest.raises(HTTPException) as error:
        await reset_staff_password(2, StaffPasswordReset(new_password="new-password"), FakeDatabase([]), {"_id": 3, "role": ADMIN})
...
async def test_password_change_invalidates_older_tokens(monkeypatch):
    db = FakeDatabase([{"_id": 2, "role": ADMIN, "is_active": True, "password_hash": hash_password("new-password"), "token_version": 1}])
    monkeypatch.setattr(core, "decode_access_token", lambda _token: {"sub": "2", "ver": 0})
    with pytest.raises(HTTPException) as error:
        await core.current_staff(db, HTTPAuthorizationCredentials(scheme="Bearer", credentials="old-token"))
